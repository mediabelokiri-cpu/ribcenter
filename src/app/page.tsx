...
export default async function HomePage() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white dark:bg-[#191919] text-[#191919] dark:text-white">
      ...
      <header className="sticky top-0 z-50 bg-white/95 dark:bg-[#191919]/95 backdrop-blur border-b border-neutral-200 dark:border-[#2A2A2A]">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-6 text-sm font-medium">
            <Link href="/" className="text-[#AF191A] dark:text-[#FFCC00] font-bold">
              Beranda
            </Link>
        ...
        {sections.map((sec) => {
          ...
          if (sec.section_key === 'hero') {
            return (
              <section
                key={sec.id}
                className="py-20 md:py-28 px-6 bg-gradient-to-b from-white via-neutral-50/50 to-white dark:from-[#191919] dark:via-[#222222] dark:to-[#191919] border-b border-neutral-200 dark:border-[#2A2A2A]"
              >
                <div className="max-w-4xl mx-auto text-center space-y-6">
                  ...
                  <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight text-[#191919] dark:text-white leading-tight">
                    {content.headline || 'Platform Informasi & Akuntabilitas Publik'}
                  </h1>
                  <p className="max-w-2xl mx-auto text-base sm:text-lg text-neutral-600 dark:text-neutral-300">
                    {content.subheadline ||
                  ...
                  <div className="flex flex-wrap justify-center gap-4 pt-4">
                    {content.cta_primary_label && (
                      <Link
                        ...
                        className="px-6 py-3 text-sm font-semibold rounded-xl bg-[#AF191A] text-white hover:bg-[#921415] transition-colors shadow-sm"
                      >
                        {content.cta_primary_label}
                      </Link>
                    )}
                    {content.cta_secondary_label && (
                      <Link
                        ...
                        className="px-6 py-3 text-sm font-semibold rounded-xl border border-[#191919] text-[#191919] hover:bg-[#191919] hover:text-white dark:border-[#FFCC00] dark:text-[#FFCC00] dark:hover:bg-[#FFCC00] dark:hover:text-[#191919] transition-colors"
                      >
                        {content.cta_secondary_label}
                      </Link>
                    )}
          ...
          if (sec.section_key === 'profile_summary') {
            return (
              <section
                key={sec.id}
                className="py-16 px-6 border-b border-neutral-200 dark:border-[#2A2A2A] bg-white dark:bg-[#191919]"
              >
                <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-10 items-center">
                  <div className="md:col-span-1 text-center md:text-left">
                    <div className="w-36 h-36 mx-auto md:mx-0 rounded-2xl bg-[#191919] border-2 border-[#AF191A] flex items-center justify-center text-4xl font-extrabold font-mono text-[#FFCC00] shadow-md">
                      RIB
                    </div>
                    <h2 className="text-xl font-bold mt-4 text-[#191919] dark:text-white">
                  ...
                  <div className="md:col-span-2 space-y-4">
                    <div className="border-l-4 border-[#AF191A] pl-4">
                      ...
                      <p className="text-sm font-medium text-neutral-800 dark:text-neutral-200 mt-1 italic">
          ...
          if (sec.section_key === 'featured_activities') {
            return (
              <section
                key={sec.id}
                className="py-16 px-6 border-b border-neutral-200 dark:border-[#2A2A2A] bg-neutral-50 dark:bg-[#1C1C1C]"
              >
                <div className="max-w-6xl mx-auto space-y-8">
                  <div className="flex justify-between items-end">
                    ...
                    <Link
                      href="/rekam-kerja"
                      className="text-xs font-semibold text-[#AF191A] dark:text-[#FFCC00] hover:underline"
                    >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {displayActivities.slice(0, getNumber(content.display_count, 3)).map((act) => (
                      <Link
                        ...
                        className="group p-5 rounded-xl border border-neutral-200 dark:border-[#2A2A2A] bg-white dark:bg-[#191919] shadow-sm flex flex-col justify-between space-y-4 hover:border-[#AF191A] dark:hover:border-[#FFCC00] transition-all"
                      >
                        <div className="space-y-2">
                          <div className="flex items-center gap-2">
                            ...
                            {act.featured && (
                              <span className="inline-block px-1.5 py-0.5 text-[10px] font-bold rounded bg-[#FFCC00] text-[#191919]">
                                ★ Unggulan
                              </span>
                            )}
                          </div>
                          <h3 className="text-base font-bold text-[#191919] dark:text-white group-hover:text-[#AF191A] dark:group-hover:text-[#FFCC00] transition-colors line-clamp-2">
                            {act.title}
                          </h3>
                          <p className="text-xs text-neutral-600 dark:text-neutral-400 line-clamp-2 leading-relaxed">{act.summary}</p>
                        </div>
                        <div className="pt-2 border-t border-neutral-100 dark:border-[#2A2A2A] text-[11px] text-neutral-500 flex justify-between">
                          <span>{act.date}</span>
                ...
                <div className="max-w-6xl mx-auto space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {articles.slice(0, getNumber(content.display_count, 3)).map((art) => (
                      <div
                        key={art.id}
                        className="p-5 rounded-xl border border-neutral-200 dark:border-[#2A2A2A] bg-white dark:bg-[#191919] shadow-sm flex flex-col justify-between space-y-4 hover:border-[#AF191A] transition-colors"
                      >
                        <div className="space-y-2">
                          ...
                          <p className="text-xs text-neutral-600 dark:text-neutral-400 line-clamp-2 leading-relaxed">{art.excerpt}</p>
                        </div>
                        <div className="pt-2 border-t border-neutral-100 dark:border-[#2A2A2A] text-[11px] text-neutral-500">
                          Penulis: {art.author}
                        </div>
                      </div>
                    ))}
                ...
                <div className="max-w-6xl mx-auto space-y-8">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                    {albums.slice(0, getNumber(content.display_count, 3)).map((alb) => (
                      <div
                        key={alb.id}
                        className="rounded-xl border border-neutral-200 dark:border-[#2A2A2A] bg-white dark:bg-[#191919] overflow-hidden shadow-sm hover:border-[#AF191A] transition-colors"
                      >
                        <div className="h-40 bg-neutral-100 dark:bg-[#252525] flex items-center justify-center text-xs text-neutral-400">
                          {alb.cover_image_url ? 'Dokumentasi Visual' : 'Album Dokumentasi'}
                        </div>
                        <div className="p-4 space-y-1">
                          <h3 className="text-sm font-bold text-[#191919] dark:text-white">{alb.title}</h3>
          ...
          if (sec.section_key === 'aspirations_cta') {
            return (
              <section
                key={sec.id}
                className="py-20 px-6 bg-[#191919] text-white border-t-4 border-[#AF191A]"
              >
                <div className="max-w-3xl mx-auto text-center space-y-6">
                  ...
                  <div>
                    <Link
                      href="/aspirasi"
                      className="inline-flex px-6 py-3 text-sm font-bold rounded-xl bg-[#FFCC00] text-[#191919] hover:bg-[#E6B800] transition-colors shadow-md"
                    >
      ...
      {/* Public Footer */}
      <footer className="py-12 px-6 bg-[#191919] border-t border-[#2A2A2A] text-xs text-neutral-400 text-center">
